...
# (Logic identical to original RouteEvaluator.calculate_cost / apply_2opt)
# ==========================================
...
def _calc_cost_jit(chrom,
    ...
    # Bit-encoded violation flags: 1=Telat Waktu, 2=Ikan Busuk, 4=Over Armada
    viol_flags = 0
...
def _apply_2opt_jit(chrom,
                    ...
                    Q_k, max_veh, fixed_cost, var_cost, big_M, t_max,
                    fish_price, q_min, alpha_arc, alpha_node):
    # In-place 2-opt with first-improvement strategy. O(1) extra space.
...
class VRPEnvironment:
    def __init__(self, file_path, num_customers=25):
...
class RouteEvaluator:
    def __init__(self, env, var_cost):
        ...
        # SoA: ekstrak field Customer ke numpy arrays sekali saja
...
# ==========================================
# TAHAP 3: HYBRID ACO + 2-OPT (ELIT 100% 2-OPT)
# ==========================================
class HybridACO:
    def __init__(self, env, evaluator, num_ants=100, max_iter=100, alpha=1, beta=2, rho=0.5, p_2opt=0.15):
        self.env = env
    ...
    def apply_2opt(self, chromosome):
        # Delegasi ke jit path (in-place numpy). Kembalikan list agar konsisten dgn API original.
        ...
        for _ in range(self.max_iter):
            ...
            for idx in range(self.num_ants):
                ...
                if random.random() < self.p_2opt:
                    tour = self.apply_2opt(tour)

                res = self.evaluator.calculate_cost(tour)
                tours.append(tour)
                penalized_costs.append(res[1])

                if res[1] < best_iter_cost:
                    best_iter_cost = res[1]
                    best_iter_idx = idx

            if best_iter_idx != -1:
                best_tour_iter = tours[best_iter_idx]
                optimized_best_tour = self.apply_2opt(best_tour_iter)
                res_opt = self.evaluator.calculate_cost(optimized_best_tour)

                tours[best_iter_idx] = optimized_best_tour
                penalized_costs[best_iter_idx] = res_opt[1]

                if res_opt[1] < best_penalized_cost:
                    best_penalized_cost = res_opt[1]
                    best_details = res_opt
...
# ==============================================================================
# FUNGSI WORKER UNTUK MULTIPROCESSING
# ==============================================================================
def run_single_repetition(file_name, num_customers, run_ke, num_ants, max_iter, alpha, beta, rho, p_2opt):
    env = VRPEnvironment(file_path=file_name, num_customers=num_customers)
    if len(env.nodes) == 0:
        return None
    evaluator = RouteEvaluator(env, var_cost=10000)
    solver = HybridACO(env, evaluator, num_ants=num_ants, max_iter=max_iter,
                       alpha=alpha, beta=beta, rho=rho, p_2opt=p_2opt)
    ...
    return {
        ...
        "waktu_eksekusi": round(t_end - t_start, 2)
    }

# ==============================================================================
# BLOK EKSEKUSI FINAL: UJI STABILITAS SKALA PENUH (25 PELANGGAN)
# ==============================================================================
if __name__ == "__main__":
    ...
    print("=" * 135)
    print(f"FINAL RUN UJI STABILITAS: HYBRID ACO + 2-OPT ({NUM_CUSTOMERS} PELANGGAN)")
    ...
    for file_name in daftar_instance:
        if not os.path.exists(file_name):
            continue
        env_check = VRPEnvironment(file_path=file_name, num_customers=NUM_CUSTOMERS)
        if len(env_check.nodes) == 0:
            continue

        for run_ke in range(1, N_REPETITIONS + 1):
            all_tasks.append((file_name, NUM_CUSTOMERS, run_ke, ANTS_FIXED, ITER_FIXED, ALPHA_BEST, BETA_BEST, RHO_BEST, P_2OPT_BEST))
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = {executor.submit(run_single_repetition, *task): task for task in all_tasks}
        ...
        for count, future in enumerate(concurrent.futures.as_completed(futures), 1):
            ...
            if res is not None:
                ...
                status_cetak = "VALID" if res['penalty'] == 0 else f"TDK VALID ({res['alasan_gagal']})"
                print(f"[{count}/{len(all_tasks)}] Selesai: {res['instance']} - Run {res['run_ke']} | Z = Rp {res['real_z']:,.0f} | Wkt = {res['waktu_eksekusi']}s | {status_cetak}")

    print("\n[SYSTEM] Seluruh pemrosesan paralel selesai. Merekapitulasi hasil...")

    # 3. Kelompokkan kembali hasil yang berantakan ke dalam format per Instance
    grouped_results = {file_name.replace(".txt", ""): [] for file_name in daftar_instance}
    for res in results_all:
        if res["instance"] in grouped_results:
            grouped_results[res["instance"]].append(res)

    # 4. Kalkulasi Metrik Akhir (MENGEMBALIKAN ARPD)
    hasil_stabilitas = []
    for instance_name, res_list in grouped_results.items():
        if not res_list:
            continue

        list_z = [r["real_z"] for r in res_list]
        list_truk = [r["armada"] for r in res_list]
        list_waktu = [r["waktu_eksekusi"] for r in res_list]
        count_valid = sum(1 for r in res_list if r["penalty"] == 0)

        alasan_gabungan = set(r["alasan_gagal"] for r in res_list if r["penalty"] != 0)
        ...
        ringkasan_masalah = "Aman" if count_valid == N_REPETITIONS else " / ".join(list(alasan_gabungan))
        ...
        hasil_stabilitas.append({
            ...
            "Total_Valid": f"{count_valid} / {N_REPETITIONS}",
            "ARPD (%)": arpd_stabilitas,
            "Alasan_Mayoritas_Gagal": ringkasan_masalah
        })

    # 5. CETAK TABEL RINGKASAN FINAL
    if len(hasil_stabilitas) > 0:
        ...
        print("\n" + "=" * 135)
        print(f"TABEL RINGKASAN UJI STABILITAS HYBRID ACO+2-OPT ({NUM_CUSTOMERS} PELANGGAN)")
        print("-" * 135)
        print(df_disp.to_string(index=False))
        print("=" * 135)

        # Simpan ke CSV
        nama_file = "FinalRun_HybridACO_2Opt_25Cust_10Reps.csv"
