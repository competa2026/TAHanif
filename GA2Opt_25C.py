...
class VRPEnvironment:
    ...
    def _load_solomon_data(self):
        try:
            ...
            print(f"Error Dataset {self.file_path}: {e}")
...
# ==========================================
# TAHAP 2: EVALUATOR (THIN WRAPPER ATAS JIT)
# ==========================================
...
# ==========================================
# TAHAP 3: HYBRID GA + 2-OPT
# ==========================================
class HybridGA:
    def __init__(self, env, evaluator, pop_size=100, max_gen=100, p_m=0.3, elitism_rate=0.1, p_2opt=0.15):
        self.env = env; self.evaluator = evaluator; self.pop_size = pop_size
        self.max_gen = max_gen; self.p_m = p_m; self.p_2opt = p_2opt
    ...
    def run(self):
        ...
        for i in range(self.pop_size):
            chrom = base_chrom.copy(); random.shuffle(chrom)
            population.append(self.apply_2opt(chrom) if i < num_enhanced else chrom)

        best_details = None; best_penalized_cost = float('inf')
        ...
        for gen in range(self.max_gen):
            ...
            for chrom in population:
                ...
                if res[1] < best_penalized_cost:
                    best_penalized_cost = res[1]; best_details = res
            ...
            for i in sorted_indices[:self.num_elites]:
                ...
                optimized_elite = self.apply_2opt(elite_chrom) # Elit dipaksa 2-Opt
                new_population.append(optimized_elite)

            total_fitness = sum(pop_fitnesses)
            while len(new_population) < self.pop_size:
                pick = random.uniform(0, total_fitness); current = 0; parent = population[-1]
                for i, f in enumerate(pop_fitnesses):
                    current += f
                    if current >= pick: parent = population[i]; break
...
# ==============================================================================
def run_single_repetition(file_name, num_customers, run_ke, pop_size, max_gen, p_m, elitism_rate, p_2opt):
    env = VRPEnvironment(file_path=file_name, num_customers=num_customers)
    if len(env.nodes) == 0:
        return None
    evaluator = RouteEvaluator(env, var_cost=10000)
    solver = HybridGA(env, evaluator, pop_size=pop_size, max_gen=max_gen,
                      p_m=p_m, elitism_rate=elitism_rate, p_2opt=p_2opt)
...
if __name__ == "__main__":
    ...
    if len(hasil_stabilitas) > 0:
        ...
        print("\n" + "=" * 135)
        print(f"TABEL RINGKASAN UJI STABILITAS HYBRID GA+2-OPT ({NUM_CUSTOMERS} PELANGGAN)")
        print("-" * 135)
        print(df_disp.to_string(index=False))
        print("=" * 135)

        # Simpan ke CSV
        nama_file = "FinalRun_HybridGA_2Opt_25Cust_10Reps.csv"
